...
//! ADD NEW: [
// listing
int load_assets_listing(json_t *root, const char *key) {
	...
	//! FIXME: realloc()
	size_t asset_num = json_array_size(node);
	if ( settings.assets) { free (settings.assets); settings.assets = NULL; }
	settings.assets = malloc(sizeof(struct asset) * asset_num);
	settings.asset_num = asset_num;
	...
	for (size_t i = 0; i < asset_num; ++i) {
		...
		//printf( "%s: %d: name = %s\n", __func__, i, name );
		ERR_RET_LN(read_cfg_int(row, "prec_save", &type.prec_save, true, 0));
		ERR_RET_LN(read_cfg_int(row, "prec_show", &type.prec_show, false, type.prec_save));


		if (strlen(name) > ASSET_NAME_MAX_LEN)
			return -__LINE__;

		//if (dict_find(dict_asset, name) == NULL) {
		...
		// re-set all
		if ( dict_add(dict_asset, name, &type) ) {
			++count;
			printf( "added asset: %s\n", name );
		}
