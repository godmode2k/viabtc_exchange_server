...
//! ADD NEW: [
// listing
static int on_cmd_listing_update(nw_ses *ses, rpc_pkg *pkg, json_t *params) {
	json_t* result = json_array();
	json_t* list_update = json_object();

	int listing = listing_update();

	json_object_set_new(list_update, "listing", json_string((listing > 0)?"updated":(listing >= 0)?"no updated":"Error"));
	json_object_set_new(list_update, "result", json_integer(listing));
	json_array_append_new(result, list_update);
...
//! ADD NEW: [
// listing
    case CMD_LISTING_UPDATE:
        log_trace("from: %s cmd listing update, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
        ret = on_cmd_listing_update(ses, pkg, params);
        if (ret < 0) {
            log_error("on_cmd_listing_update %s fail: %d", params_str, ret);
        }
        break;
// ]
