...
//! ADD NEW: [
// listing
int listing_update(void) {
	const char* path = "/mnt/data/viabtc/viabtc_exchange_server/matchengine/config.json";
	int count = 0;
	int ret = 0;
	...
	if ( (ret = load_assets_listing(root, "assets")) < 0 ) {
		printf("load assets config fail: %d\n", ret);
		return -__LINE__;
	}
	//printf( "listing: asset: count: %d\n", ret );
	count += ret;

	if ( (ret = load_markets_listing(root, "markets")) < 0 ) {
		printf("load markets config fail: %d\n", ret);
		return -__LINE__;
	}
	//printf( "listing: market: count = %d\n", ret );
	count += ret;
