...
//! ADD NEW: [
// Check process running inside Docker
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>
// ]
...
//! ADD NEW: [
// Check process running inside Docker
// PID_MAX_DEFAULT 0x8000 (32768)
{
    printf( "Redis Host (private or public?): %s:%d\n", addr.host, addr.port );

    // strlen("/proc/") + 5 + strlen("cgroup") = 17
    char buf_pid_path[17+1] = { 0, };
    char buf[255] = { 0, };
    struct stat st;
    pid_t pid = getpid();

    snprintf( buf_pid_path, sizeof(buf_pid_path), "/proc/%d/cgroup", pid );
    //printf( "this PID path = %s\n", buf_pid_path );

    if ( stat(buf_pid_path, &st) == 0 ) {
        FILE* fp = fopen( buf_pid_path, "r" );

        if ( fgets(buf, sizeof(buf), fp) != NULL ) {
            if ( strstr(buf, "docker") != NULL ) {
                printf( "this PID (%d) running inside Docker.\n", pid );

                redis_sentinel_node* curr = context->list;
                addr.host = strdup( curr->addr.host );
                // use Redis port
                //addr.port = curr->addr.port; // Sentinel port

                // Docker container
                //  - redis: 0.0.0.0:6380->6379/tcp, :::6380->6379/tcp
                //  - sentinel: 6379/tcp, 0.0.0.0:26380->26379/tcp, :::26380->26379/tcp
                char buf_port[5+1] = { 0, };
                snprintf( buf_port, sizeof(buf_port), "%d", curr->addr.port );
                snprintf( buf_port, sizeof(buf_port), "%s", buf_port + 1 );
                //printf( "sentinel port: %d, redis port: %s\n", curr->addr.port, buf_port );
                addr.port = atoi( buf_port ); // 26381 (sentinel public) -> 6381 (redis public)

                printf( "now, Docker container (private) -> Redis Host (public): %s:%d\n", addr.host, addr.port );
	            printf( "\n" );
            }
        }
